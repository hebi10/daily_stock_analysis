...
> Note: `POST /api/v1/analysis/analyze` accepts `report_language=ko|en|zh` (legacy-compatible alias `reportLanguage`). When omitted, it falls back to global `REPORT_LANGUAGE`. This parameter is request-scoped only and influences report output language for this run, including `report.meta.report_language` in responses.
...
> Note: `POST /api/v1/analysis/market-review` also accepts `report_language=ko|en|zh` / `reportLanguage` to set report language for that request. If omitted, it falls back to global `REPORT_LANGUAGE`; Bot/CLI/manual `/market-review` calls keep using global config and do not carry request-level override.
