...
| `REPORT_LANGUAGE` | 리포트 출력 언어: `ko`(기본 한국어) / `en`(영어) / `zh`(중국어). Prompt, 템플릿, 알림 fallback, Web 리포트 페이지 고정 문구에 함께 반영됩니다. 저장소의 `00-daily-analysis.yml`은 이 변수를 명시적으로 매핑하므로 Actions Secrets/Variables에 설정하면 바로 적용됩니다 | 可选 |
...
- Web 语言状态采用两层机制：`dsa.uiLanguage`（浏览器持久化）与 `REPORT_LANGUAGE`（报告输出）解耦。
  ...
  - `REPORT_LANGUAGE`는 리포트 텍스트, 종목명 현지화, 리포트 페이지 고정 문구를 제어합니다(`ko` / `en` / `zh`).
...
> 说明：`POST /api/v1/analysis/analyze`는 `report_language=ko|en|zh`를 지원하며 `reportLanguage` 별칭도 호환합니다. 전달하지 않으면 전역 `REPORT_LANGUAGE` 또는 환경의 `Config.report_language`로 fallback됩니다. 이 필드는 이번 분석의 리포트 텍스트, `report.meta.report_language`, 저장된 표시 내용에만 영향을 주며 런타임 설정으로 저장되지는 않습니다.
...
> 说明：`POST /api/v1/analysis/market-review`도 `report_language=ko|en|zh`를 지원합니다(`reportLanguage` 별칭 포함). 전달하지 않으면 전역 `REPORT_LANGUAGE`로 fallback됩니다. 이 매개변수는 이번 복기 리포트 텍스트와 구조화 반환 필드의 언어 관련 내용에만 영향을 줍니다. Bot, schedule, CLI 또는 버튼으로 실행하는 `main.py --market-review`는 계속 전역 설정을 사용합니다.
