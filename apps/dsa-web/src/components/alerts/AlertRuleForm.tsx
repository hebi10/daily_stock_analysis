...
function optionsForScope(scope: AlertTargetScope, language: FeatureLanguage) {
...
export const AlertRuleForm: React.FC<AlertRuleFormProps> = ({ onSubmit, isSubmitting = false }) => {
  ...
  const text = ALERT_FORM_TEXT[featureLanguage];
  ...
  const alertTypeOptions = useMemo(() => optionsForScope(targetScope, featureLanguage), [featureLanguage, targetScope]);
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (targetScope === 'single_symbol') {
      ...
      if (!targetValidation.valid) {
        setFormError(featureLanguage === 'en' ? text.invalidStockCode : (targetValidation.message ?? text.invalidStockCode));
        return;
      }
    ...
    if (targetScope === 'market') {
      return (
        <Select
          ...
          value={marketRegion}
          options={ALERT_MARKET_REGION_OPTIONS[featureLanguage]}
          disabled={isSubmitting}
      ...
      <form className="space-y-4" noValidate onSubmit={(event) => void handleSubmit(event)}>
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <Select
            ...
            value={targetScope}
            options={ALERT_TARGET_SCOPE_OPTIONS[featureLanguage]}
            disabled={isSubmitting}
          ...
          <Select
            label={text.severity}
            value={severity}
            options={ALERT_SEVERITY_OPTIONS[featureLanguage]}
            disabled={isSubmitting}
        ...
        {alertType === 'price_cross' ? (
          <div className="grid gap-4 md:grid-cols-2">
            <Select
              ...
              options={ALERT_PRICE_DIRECTION_OPTIONS[featureLanguage]}
              disabled={isSubmitting}
        ...
        {alertType === 'price_change_percent' ? (
          <div className="grid gap-4 md:grid-cols-2">
            <Select
              ...
              options={ALERT_CHANGE_DIRECTION_OPTIONS[featureLanguage]}
              disabled={isSubmitting}
        ...
        {alertType === 'ma_price_cross' ? (
          <div className="grid gap-4 md:grid-cols-2">
            <Select
              ...
              options={ALERT_THRESHOLD_DIRECTION_OPTIONS[featureLanguage]}
              disabled={isSubmitting}
        ...
        {alertType === 'macd_cross' ? (
          <div className="grid gap-4 md:grid-cols-4">
            <Select
              ...
              options={ALERT_CROSS_DIRECTION_OPTIONS[featureLanguage]}
              disabled={isSubmitting}
        ...
        {alertType === 'portfolio_stop_loss' ? (
          <Select
            ...
            value={stopLossMode}
            options={ALERT_STOP_LOSS_MODE_OPTIONS[featureLanguage]}
            disabled={isSubmitting}
        ...
        {alertType === 'market_light_status' ? (
          <div className="space-y-2">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              {ALERT_MARKET_LIGHT_STATUS_OPTIONS[featureLanguage].map((option) => (
                <Checkbox
                  key={option.value}
                  label={option.label}
