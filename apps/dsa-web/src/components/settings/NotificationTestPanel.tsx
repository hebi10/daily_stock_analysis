...
function getChannelOptions(language: UiLanguage): Array<{ value: NotificationTestChannel; label: string }> {
  ...
  return [
    { value: 'wechat', label: isKo ? '±â¾÷ À§Ãª' : isEn ? 'WeCom' : 'Ðê?Ú°ãá' },
    { value: 'feishu', label: isKo ? 'Feishu Webhook' : isEn ? 'Feishu Webhook' : '?? Webhook' },
    { value: 'telegram', label: 'Telegram' },
    { value: 'email', label: isKo ? 'ÀÌ¸ÞÀÏ' : isEn ? 'Email' : '?Ëì' },
    { value: 'pushover', label: 'Pushover' },
    { value: 'ntfy', label: 'ntfy' },
    { value: 'gotify', label: 'Gotify' },
    { value: 'pushplus', label: 'PushPlus' },
    { value: 'serverchan3', label: 'ServerChan3' },
    { value: 'custom', label: isKo ? '»ç¿ëÀÚ Webhook' : isEn ? 'Custom Webhook' : 'í»ïÒ? Webhook' },
        ...
        <div className="space-y-3">
          <InlineAlert
            ...
            message={(
              <span>
                {result.message}
                {typeof result.latencyMs === 'number' ? ` Â· ${result.latencyMs} ms` : ''}
                {result.errorCode ? ` Â· ${result.errorCode}` : ''}
              </span>
            )}
          />
